...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'core':
				dialogue = CoolUtil.coolTextFile(Paths.txt('core/diag'));
			case 'punchtune':
				dialogue = CoolUtil.coolTextFile(Paths.txt('punchtune/diag'));
			case 'hardcore':
				dialogue = CoolUtil.coolTextFile(Paths.txt('hardcore/diag'));
		}
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'core' | 'punchtune' | 'hardcore':
				{
						defaultCamZoom = 0.6; //8
						curStage = 'stagecore';
	
						var stageFront:FlxSprite = new FlxSprite(-1120, -620).loadGraphic(Paths.image('corebg'));
						stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
						stageFront.updateHitbox();
						stageFront.antialiasing = true;
						...
						amanddica = new FlxSprite(0, 0);
						amanddica.frames = Paths.getSparrowAtlas('AMANDDICA', 'week2');
						amanddica.animation.addByPrefix('bop', "BOP", 24, false); 
						amanddica.scrollFactor.set(1, 1);
						amanddica.antialiasing = true;
						add(amanddica);

						screencore = new FlxSprite(0, 0).loadGraphic(Paths.image('corebgSCREEN', 'shared'));
						screencore.scrollFactor.set(0, 0);
						screencore.cameras = [camHUD];
				}
			case 'corexpurgation':
				{
						defaultCamZoom = 0.6; //8
						...
						var stageFront:FlxSprite = new FlxSprite(-1120, -620).loadGraphic(Paths.image('corebgmadness'));
						stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
						stageFront.updateHitbox();
						stageFront.antialiasing = true;
						stageFront.scrollFactor.set(1, 1);
						...
						amanddica = new FlxSprite(0, 0);
						amanddica.frames = Paths.getSparrowAtlas('AMANDDICA_M', 'week2');
						amanddica.animation.addByPrefix('bop', "BOP", 24, false); 
						amanddica.scrollFactor.set(1, 1);
						amanddica.antialiasing = true;
						add(amanddica);

						screencore = new FlxSprite(0, 0).loadGraphic(Paths.image('corebgSCREENMADNESS', 'shared'));
						screencore.scrollFactor.set(0, 0);
						screencore.cameras = [camHUD];
				}
			case 'chair-battle':
				{
						...
						var stageFront:FlxSprite = new FlxSprite(-1000, -620).loadGraphic(Paths.image('casaantiga'));
						stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
						stageFront.updateHitbox();
						stageFront.antialiasing = true;
						stageFront.scrollFactor.set(1, 1);
		...
		if (curStage == 'stagecore' || curStage == 'stagecore')
			add(screencore);
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if(daStrumTime >= startTime)
				{
					...
					var swagNote:Note = new Note(daStrumTime, (!randomNotes ? daNoteData : daRandomNoteData), oldNote, false, (!gottaHitNote ? dad.noteSkin : ""), daType);
					swagNote.sustainLength = songNotes[2];
					swagNote.scrollFactor.set(0, 0);
					swagNote.altNote = songNotes[4];
					...
					for (susNote in 0...Math.floor(susLength))
					{
						...
						var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, (!randomNotes ? daNoteData : daRandomNoteData), oldNote, true, (!gottaHitNote ? dad.noteSkin : ""), daType);
	...
	override public function update(elapsed:Float)
	{
		...
		if (curSong.toLowerCase() == 'hardcore')
		{
		...
		else if (curSong.toLowerCase() == 'from-hell')
			{
					exfloatshit += 0.016;
					floatshit -= 0.016;
			}
			...
			/*
		if (curSong.toLowerCase() == 'hardcore' && curStep >= 2496 && curStep < 3008 && camZooming && FlxG.camera.zoom < 1.35)
			{
				rotatoCam = 0.06;
			}
			*/
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection/* && camFollow.x != boyfriend.getMidpoint().x - 100*/ && (!pointAtGF && !camPointEventEnabled))
			{
				...
				switch (curStage)
				{
					...
					case 'schoolEvil':
						...
						camFollowY = boyfriend.getMidpoint().y - 100;
					case 'stagecasa':
						camFollowY = boyfriend.getMidpoint().y - 300;
					case 'philly':
		...
		if (health <= 0)
		{
			if (dad.curCharacter == "core")
				{
					new FlxTimer().start(0.4, function(tmr:FlxTimer)
						{
							FlxG.sound.play(Paths.soundRandom('core', 1, 4), FlxG.random.float(1.5, 1.5));
						});
				}

			if (dad.curCharacter == "core-madness")
				{
					new FlxTimer().start(0.4, function(tmr:FlxTimer)
						{
							FlxG.sound.play(Paths.soundRandom('core', 1, 8), FlxG.random.float(1.5, 1.5));
						});
				}
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			else
			{
				...
				#if cpp
				if(OpenFlAssets.exists(Paths.json(PlayState.storyPlaylist[0].toLowerCase() + "/" + PlayState.storyPlaylist[0].toLowerCase() + (storyDifficulty != 3 ? "-events" : "-funky-events"))))
				...
				if (curSong == 'Corexpurgation') 
					{
						...
						if (curStep == 2646)
							{
								FlxTween.tween(camHUD, {angle:2.5}, 2.0, {ease: FlxEase.expoInOut});
						...
						if (curStep == 2912)
							{
								FlxTween.tween(camHUD, {angle:0}, 2.0, {ease: FlxEase.expoInOut});
								drainHealth = false;
							}
						//nao mais haha
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'punchtune' && curStep >= 256 && curStep < 896 && camZooming && FlxG.camera.zoom < 1.35)
			{
				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.03;
			}

		if (curSong.toLowerCase() == 'punchtune' && curStep >= 1016 && curStep < 1664 && camZooming && FlxG.camera.zoom < 1.35)
			{
				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.04;
			}

		if (curSong.toLowerCase() == 'hardcore' && curStep >= 1216 && curStep < 1728 && camZooming && FlxG.camera.zoom < 1.35)
			{
				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.03;
				defaultCamZoom = 0.7;
		...
		else if (curSong.toLowerCase() == 'hardcore' && curStep >= 1984 && curStep < 2489 && camZooming && FlxG.camera.zoom < 1.35)
			{
				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.04;
				defaultCamZoom = 0.75;
		...
		else if (curSong.toLowerCase() == 'hardcore' && curStep >= 2496 && curStep < 3008 && camZooming && FlxG.camera.zoom < 1.35)
			{
		...
		if (curSong.toLowerCase() == 'corexpurgation' && curStep >= 320 && curStep < 576 && camZooming && FlxG.camera.zoom < 1.35)
			{
				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.04;
			}

		if (curSong.toLowerCase() == 'corexpurgation' && curStep >= 608 && curStep < 1632 && camZooming && FlxG.camera.zoom < 1.35)
			{
				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.04;
			}

		if (curSong.toLowerCase() == 'corexpurgation' && curStep >= 1632 && curStep < 1888 && camZooming && FlxG.camera.zoom < 1.35)
			{
				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.05;
			}

		if (curSong.toLowerCase() == 'corexpurgation' && curStep >= 2144 && curStep < 2656 && camZooming && FlxG.camera.zoom < 1.35)
			{
				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.06;
			}
		...
		{
			case 'stagecore':
				amanddica.animation.play('bop', true);
